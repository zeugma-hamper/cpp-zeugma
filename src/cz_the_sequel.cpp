...
class VideoCursor  :  public Zeubject
{ public:
  Node *node;
  Renderable *renderable;
  ZoftVect position;
  f64 default_size;
  VideoCursor (f64 sz)  :  Zeubject (),
                           node (nullptr),
                           renderable (nullptr),
                           default_size (sz)
    {
  ...
  Node *RemoveNode ()
  {
    Node *ret = std::exchange (node, nullptr);
    renderable = nullptr;

    //this leaves the translation zoft in the grappler stuck with latch
    //guts i think, but i don't have anyway to directly access it.
    position.MakeGutless ();

    return ret;
  }

  void InstallNode (Node *_node, Renderable *_renderable)
  {
    assert (node == nullptr);
    ...
    node->Scale(default_size);
    position.MakeBecomeLikable();
...
class dead_zone final : public charm::Application
{
 public:
  ...
  void CursorHarden (ZESpatialHardenEvent *e);
  void CursorSoften (ZESpatialSoftenEvent *e);

 protected:
  GLFWwindow *window;
  Layer *m_scene_graph_layer;
  std::vector<FilmInfo> m_film_info;

  std::vector<VideoCursor *> m_cursors;
...
static MattedVideoRenderable *MakeRandomMatte (std::vector<FilmInfo> const &_films)
{
  u64 film_index = u64(std::floor (_films.size () * drand48()));
  FilmInfo const &film = _films[film_index];
  u64 clip_index = u64(std::floor (film.clips.size () * drand48 ()));
  ClipInfo const &clip = film.clips[clip_index];
  auto *matte = new MattedVideoRenderable (film, clip);

  printf ("creating new matte %s : %s\n", film.abbreviation.c_str (),
          clip.name.c_str ());
...
bool dead_zone::StartUp ()
{
  ...
  m_film_info = ReadFilmInfo ("../configs/film-config.toml");
  assert (m_film_info.size () > 0);

  std::vector<FilmGeometry> geom = ReadFileGeometry("../configs/mattes.toml");
  assert (geom.size () > 0);
  MergeFilmInfoGeometry(m_film_info, geom);

  PlatonicMaes *maes = FindMaesByName ("front");
  for (int q = 0  ;  q < 2  ;  ++q)
    {
      Node *node = new Node ();
      MattedVideoRenderable *matte = MakeRandomMatte(m_film_info);
      node->AppendRenderable(matte);

      VideoCursor *vc = new VideoCursor (0.015 * maes -> Height ());
      vc->InstallNode(node, matte);
      vc->position = maes->Loc ();

      m_scene_graph_layer->GetRootNode()->AppendChild(vc->node);
      m_cursors.push_back (vc);
    }
...
{ PlatonicMaes *close_m = NULL;
  ...
  const std::string &cursor_name = e -> Provenance ();
  VideoCursor *cursor = NULL;
  VideoCursor *bachelor_cursor = NULL;
  for (VideoCursor *vc  :  m_cursors)
    {
      const std::string &nm = vc->Name ();
      if (nm.empty ())
        bachelor_cursor = vc;
      else if (nm  ==  cursor_name)
        { cursor = vc;  break; }
    }
  ...
  if (! cursor)
    {
      if (! bachelor_cursor)
        return;
      cursor = bachelor_cursor;
      bachelor_cursor -> SetName (cursor_name);
    }

  i32 const cnt = NumMaeses ();
  for (i32 q = 0  ;  q < cnt  ;  ++q)
    {
  ...
  if (close_m)
    {
      cursor->position = close_p;
      cursor->renderable->SetOver (close_m->Over ());
      cursor->renderable->SetUp (close_m->Up ());
    }
}

void dead_zone::CursorHarden (ZESpatialHardenEvent *e)
{
  (void)e;
}

void dead_zone::CursorSoften (ZESpatialSoftenEvent *e)
{
  (void)e;

  auto found = std::find_if (m_cursors.begin (), m_cursors.end (),
                             [e] (VideoCursor *vc) { return vc->Name () == e->Provenance(); });

  if (found == m_cursors.end ())
    return;

  VideoCursor *vc = *found;
  Node *node = vc->RemoveNode();
  (void)node; //already in the scene graph

  Node *new_node = new Node ();
  MattedVideoRenderable *matte = MakeRandomMatte(m_film_info);
  new_node->AppendRenderable(matte);

  vc->InstallNode(new_node, matte);
}
...
int main (int, char **)
{
  ...
  PlatonicMaes *maes = zone . FindMaesByName ("front");
