...
template<size_t N>
static bgfx::ShaderHandle makeShaderFromArray (uint8_t const (&data)[N])
{
  return bgfx::createShader (bgfx::makeRef (data, uint32_t(N)));
}
	...
	: m_fontManager(_fontManager)
{
	...
	m_basicProgram
    = bgfx::createProgram(makeShaderFromArray (vs_font_basic),
                          makeShaderFromArray (fs_font_basic),
                          true);

  m_distanceProgram
    = bgfx::createProgram(makeShaderFromArray (vs_font_distance_field),
                          makeShaderFromArray (fs_font_distance_field),
                          true);
  m_distanceSubpixelProgram
    = bgfx::createProgram(makeShaderFromArray (vs_font_distance_field_subpixel),
                          makeShaderFromArray (fs_font_distance_field_subpixel),
                          true);
